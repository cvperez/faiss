...
#SBATCH --output=/mnt/common/cvazquezperezdelacru/IOWARP/faiss/contrib/iowarp/benchmarks/results/logs/%j_qps.out
...
#
# Outputs:
#   benchmarks/results/logs/<jobid>_qps.out   this job's log
#   benchmarks/results/qps_<volume>.csv       appended by bench_ivf_qps
#
# Examples:
#   mkdir -p benchmarks/results/logs   # sbatch needs the --output dir to exist
#   sbatch benchmarks/scripts/sbatch_bench.sh
#   sbatch --export=ALL,FAISS_VOLUME=ondisk_nb50M,FAISS_BACKENDS="mmap iowarp" \
#       benchmarks/scripts/sbatch_bench.sh
...
bash "$ROOT/benchmarks/scripts/00_env_ares.sh"
...
TELEMETRY_CSV="$ROOT/benchmarks/results/telemetry_${FAISS_VOLUME}_${SLURM_JOB_ID:-local}.csv"
...
bash "$ROOT/benchmarks/scripts/30_run_bench.sh" "$FAISS_VOLUME" $FAISS_BACKENDS
