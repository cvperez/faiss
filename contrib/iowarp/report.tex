...
For comparison, Table~\ref{tab:mmap} shows the original memory-mapped
\texttt{OnDiskInvertedLists} baseline over the same index sizes, using
the same protocol.
...
\toprule
Index size & nprobe & QPS (cold) & QPS (warm) & page-ins (cold) & page-ins (warm) \\
\midrule
4.8 GB & 64  & 28.2 & 238 - 360 & 46{,}144      & $<$ 700 \\
24 GB  & 128 & 10.8 & 93        & 213{,}276     & $<$ 200 \\
49 GB  & 256 & 2.0  & 5.5 - 6.4 & 756{,}611     & $\approx$ 535{,}000 \\
87 GB  & 256 & 0.4  & 0.4       & 2{,}811{,}912 & $\approx$ 2{,}725{,}000 \\
\bottomrule
\end{tabular}
\caption{Memory-mapped \texttt{OnDiskInvertedLists} baseline: throughput and
page-ins versus index size, same node and protocol as Table~\ref{tab:qps}.}
\label{tab:mmap}
\end{table}

The behaviour of the baseline changes completely with size. For the 4.8 and 24
gigabyte indexes, the first (cold) pass is slow and pays hundreds of thousands
of page-ins, but once the data is warm in the page cache the page-ins almost
vanish and throughput recovers sharply, reaching 93 QPS at 24 gigabytes. For
the 49 and 87 gigabyte indexes, however, the warm passes never recover: the
number of page-ins stays in the hundreds of thousands, even millions, and
throughput stays at a few queries per second or below. This is page-cache
thrashing: the index no longer fits in memory, so every pass keeps evicting and
reloading the same lists from disk.

Comparing Table~\ref{tab:mmap} with Table~\ref{tab:qps} shows a clear
crossover. While the index fits in RAM, the warm memory-mapped baseline is
faster than the CTE-based implementation (93 versus about 27 QPS at 24
gigabytes), because a warm page is scanned directly, without any copy. Once the
index no longer fits in RAM, the situation reverses: the CTE-based
implementation keeps a stable throughput with zero page-ins (7.9 QPS at 49
gigabytes and 4.0 QPS at 87 gigabytes), whereas the memory-mapped baseline
collapses to 6.4 and 0.4 QPS respectively.
...
93 QPS versus about 27 QPS at 24 gigabytes). The difference is exactly
